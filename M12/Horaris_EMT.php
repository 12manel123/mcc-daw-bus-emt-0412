...
<h3>Email: maca494@vidalibarraquer.net Telefon: 611491869</h3>
    ...
    <div class="inicio">
    	<img src="images/logoemt_2.png" class="imglogo">
        <h1>Detalls de Línies </h1>
        <div>
    <form action="Horaris_EMT.php" method="POST">
    	<h4><span class="textodato">Linea: </span>
	<select name="line" class="content-select">
		<?php
		foreach ($linea as $key => $value) {echo '<option value="'.$key.'">'.$key.'</option>';}
		?>
	</select>
    	<span class="textodato">Dia:</span> 
	<select name="dia" class="content-select">
		<?php
		foreach ($linea[1] as $key => $value) {echo '<option value="'.$key.'">'.$key.'</option>';}
		?>
	</select>
    	<span class="textodato">Direcció: </span>
	<select name="direccio" class="content-select">
		<?php
		foreach ($linea[0]["feiners"] as $key => $value) {echo '<option value="'.$key.'">'.$key.'</option>';}
		?>
	</select>
	...
	<select name="parada" class="content-select">
		<?php
		$suma = 0;
		foreach ($linea[0]["zona"] as $key => $value) {echo '<option value="'.$suma.'">'.$value.'</option>';$suma++;}
		?>
	</select>
</h4>
...
if(isset($_POST["enviar"])){
	...
	if($_POST["direccio"]=="anada"){
		echo " <br><h3>Direcció Vilafranca -> El vendrell -> Torredembarra -> Tarragona</h3></h2>";
	}
	if($_POST["direccio"]=="tornada"){
		echo " <br><h3>Direcció Tarragona -> Torredembarra -> El vendrell -> Vilafranca</h3></h2>";
	}
